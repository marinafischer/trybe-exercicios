...
//loteria - desafio lançado durante a aula
...
let loteriaNumbers = [n1, n2, n3, n4, n5, n6]
let myNumers = [02,25,27,25,10,17]
console.log(loteriaNumbers)
for (let i = 0; i<loteriaNumbers.length; i++) {
  for (let j=0; j<myNumers.length; j++) {
    if(myNumers[j] == loteriaNumbers[i]) {
      console.log(`você acertou o múmero ${loteriaNumbers[i]}`)
    } else {
      console.log(`o número ${loteriaNumbers[i]} foi sorteado e não foi escolhido`)
    }
  }
}